...
SELECT
	city_name,
	ROUND((population*0.25)/1000000,2) as customer_consumers_in_millions,
	city_rank
FROM city
ORDER BY 2 DESC;
...
SELECT 
	...
	last_month_sale,
	ROUND(
		(curr_month_sale - last_month_sale)::numeric / last_month_sale::numeric * 100,2
		) AS growth_ratio
FROM growth_ratio	
WHERE
...
(
	SELECT
		...
		estimated_rent,
		ROUND
			(((population * 0.25)/1000000)::numeric, 3)
